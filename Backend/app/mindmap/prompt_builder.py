    ...
    def build_top_level_messages(
        ...
        *,
        ...
        payload = {
            ...
            "course": course.dict(),
            "requirements": [
                "必须生成课程根节点、一级知识模块，以及每个一级知识模块下的二级子节点。",
                "一级知识模块生成 4 到 6 个；每个一级知识模块生成 2 个二级子节点，材料不足时最多生成 1 个。",
                "总节点数控制在 22 个以内，summary 每个不超过 80 个汉字。",
                "二级子节点应体现定义、定理、方法、例题类型、应用场景或易错点，不要只重复一级标题。",
                "rootNodeId 必须是 node-root。",
                "根节点 parentId 必须是 null，title 使用课程名。",
                "每个非根节点 parentId 必须指向已有节点。",
                "一级节点 depth=1，二级节点 depth=2；如果某一级节点仍可继续细分，hasMoreChildren=true。",
